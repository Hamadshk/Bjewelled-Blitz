...
void myLines(int x1, int y1, int x2, int y2, int red, int green, int blue) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen = CreatePen(PS_SOLID, 10, RGB(red, green, blue)); //2 is the width of the pen
...
void myLine(int x1, int y1, int x2, int y2, int red, int green, int blue) //use three 3 integers if you want colored lines.
{
	HWND console_handle = GetConsoleWindow();
	HDC device_context = GetDC(console_handle);

	//change the color by changing the values in RGB (from 0-255) to get shades of gray. For other colors use 3 integers for colors.
	HPEN pen = CreatePen(PS_SOLID, 3, RGB(red, green, blue)); //2 is the width of the pen
	SelectObject(device_context, pen);
	MoveToEx(device_context, x1, y1, NULL);
	LineTo(device_context, x2, y2);
	ReleaseDC(console_handle, device_context);
...
bool isCursorKeyPressed(int& whichKey)   //whichKey passed as reference.... 
{
	...
	if (key == 1)
	{
		...
		whichKey = 5;   // 4 if down key is pressed
		return true;
	}
	/*if (std::cin.get() == '\n')
	{
